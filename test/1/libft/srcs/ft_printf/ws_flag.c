...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/05/29 17:45:59 by acoupleu          #+#    #+#             */
...
void		print_wsminus(wchar_t *str, t_ws *ws, t_flag *f, t_env *env)
{
	int		count;
	int		wlen;
...
static void	do_wstring2(wchar_t *str, t_flag *f, t_env *env, t_ws *ws)
{
	if (f->acc < 0)
		while ((f->size)-- > 0)
			(env->n) += count_putchar(' ');
